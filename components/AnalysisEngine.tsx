...
export default function AnalysisEngine({ isOpen = true, onToggle, showStrategies = true }: AnalysisEngineProps) {
    const { settings, isLoaded, toggleVisibleStrategy, toggleAllStrategies, activeAsset } = useUserStore();
        ...
        <div className="flex flex-col h-full bg-card overflow-hidden">
            ...
            <div className={`flex-1 p-5 space-y-2 overflow-y-auto custom-scrollbar transition-opacity duration-300 ease-in-out ${isOpen ? 'opacity-100 delay-100' : 'opacity-0 pointer-events-none'}`}>
                ...
                <div className="min-w-[280px] space-y-4">
                    ...
                    {activeAsset && (
                        <div className="bg-muted/10 p-2 rounded-lg border border-border/50">
                            <div className="flex items-center mb-2 px-1">
                                <h3 className="text-[10px] text-muted-foreground uppercase tracking-wider font-bold mr-1">Market Trend</h3>
                                <InfoTooltip content="Shows the current price trend across 4 timeframes (15m, 1H, 4H, 1D). Trends are determined by Price vs EMA50 (Above = Bullish, Below = Bearish)." />
                            </div>
                            <TrendDashboard
                                symbol={activeAsset}
                            />
                    ...
                    {/* Strategies (Screener Columns) - Only shown if requested */}
                    {showStrategies && (
                        <FilterSection
                            id="strategies"
                            title="Strategies"
                            ...
                            description="Filter assets based on technical strategies. Active strategies will scan the market for entry signals."
                        >
