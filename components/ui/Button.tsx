...
// I'll stick to standard HTMLButtonElement for now and skip Radix Slot unless I see it in package.json.
// "lucide-react" IS installed.

export interface ButtonProps
    extends React.ButtonHTMLAttributes<HTMLButtonElement> {
    variant?: 'default' | 'destructive' | 'outline' | 'ghost' | 'link' | 'emerald' | 'secondary';
        ...
        return (
            <button
                className={cn(
                    ...
                    variant === 'emerald' && 'bg-emerald-600 text-white shadow-sm hover:bg-emerald-500',
                    // Sizes
                    size === 'default' && 'h-9 px-4 py-2',
                    size === 'sm' && 'h-8 rounded-md px-3 text-xs',
                    size === 'xs' && 'h-7 rounded-sm px-2 text-[10px]',
